...
function renderTitle() {
  ...
  screen.innerHTML = `<h2>What will you make of a lifetime?</h2>
...
function render() {
  ...
  const heading = `<p class="chapter-label">Chapter ${state.chapter + 1} of ${CHAPTERS.length} · Age ${chapter.age}</p><h2>${chapter.title}</h2>`;

  if (state.phase === "dialogue") {
    screen.innerHTML = `${heading}<p class="lede">${chapter.dialogue[state.dialogueIndex]}</p>${statsMarkup()}
    ...
    screen.innerHTML = `${heading}<p>Choose how to spend this chapter. You may select one activity.</p>${statsMarkup()}
    ...
    screen.innerHTML = `${heading}<p class="lede">${chapter.milestone}</p>${statsMarkup()}
    ...
    screen.innerHTML = `${heading}<p class="lede">${choice.result}</p>${statsMarkup()}
...
function renderEnding() {
  ...
  screen.innerHTML = `<p class="chapter-label">Age 21 · Your ending</p><h2>${ending.title}</h2>
